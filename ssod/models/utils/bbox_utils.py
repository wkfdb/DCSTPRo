...
def filter_invalid2(bbox, label=None, logit=None, min_size=0):
    if min_size is not None:
        bw = bbox[:, 2] - bbox[:, 0]
        bh = bbox[:, 3] - bbox[:, 1]
        valid = (bw > min_size) & (bh > min_size)
...
def filter_invalid_topk(bbox, label=None, logit=None, thr=0.0, min_size=0):
    if logit is not None:
        topk_scores, idxs = torch.topk(logit,3,dim=-1)
        valid = topk_scores.sum(-1) > thr
        bbox = bbox[valid]
        if label is not None:
            label = label[valid]
    if min_size is not None:
...
def filter_invalid_dynamic(bbox, label=None, score=None, thr=0.0, min_size=0):
    if score is not None:
        class_wise_thr = thr[label]
        valid = score > class_wise_thr
        bbox = bbox[valid]
        if label is not None:
            label = label[valid]
    if min_size is not None:
